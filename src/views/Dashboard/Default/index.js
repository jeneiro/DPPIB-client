...
//Modal.setAppElement('#Default');
const Default = () => {
    const [record, setRecord] = useState([]);
    const [modalSwitch, setModalSwitch] = useState(false);
    const [modalContent, setModalContent] = useState('');
    useEffect(() => {
        const getRecords = `${webapibaseurl}release/`;
        axios.get(getRecords).then((response) => {
            setRecord(response.data);
            // localStorage.setItem("investorData", JSON.stringify(response.data) );;
            return response;
        });
    }, []);
    ...
    const columns = [
        ...
        { field: 'ocid', headerName: 'OCID', width: 200 },
        { field: 'awardStatus', headerName: 'AwardStatus', width: 200 },

        { field: 'awardDate', headerName: 'Award Date', width: 220 },

        {
            ...
            renderCell: (cellValue) => {
                const handleSelect = (e) => {
                    ...
                    const getRecords = `${webapibaseurl}release/json-download/${id}`;
